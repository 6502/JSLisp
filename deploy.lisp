           ...
           (dolist (k (sort (keys result)))
             (let ((f (aref result k)))
               (when (and f (. f prototype) (. f prototype %class))
                 (unless classes
                   (setf classes true)
                   (push "d$$$42_constructors$42_={};" output))
                 (let* ((ix (index "$$" k))
                        (mod (+ (demangle (+ "$$" (slice k 0 ix))) ":"))
                        (name (demangle (slice k 0 -12))) ; remove "_constructor"
                        (cargs (json (slice (. f prototype %class) 1)))
                        (newf (+ (slice k 0 ix) "$$new_" (slice (mangle name) 2))))
                   (when (= mod ":") (setf mod ""))
                   (push ~"d$$$42_constructors$42_[\"{mod}{name}\"]=[{cargs},f{newf}];" output))
      ...
      (dolist (x todo)
        ...
        (dolist (fn (or (and (symbol-function x)
          ...
          (unless (aref result fn)
            (let* ((i (index "$$" fn))
                   (y (intern (demangle fn) (demangle (+ "$$" (slice fn 0 i))))))
              (push y new-todo)))))
      (setf todo new-todo))))
