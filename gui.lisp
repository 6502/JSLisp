...
(defun ask-color (prompt color cback)
  ...
  (with-window (w (100 100 300 320 :title prompt :close false)
    ...
    (setf cursor.width 20)
    (setf cursor.height 20)
    (with-canvas cursor
      (begin-path)
      ...
      (fill-style "#FFFFFF")
      (fill))
    (let ((colv (/ (max (first color) (second color) (third color))
                   255))
          (colx null)
          (coly null)
          (colerr null)
      ...
      (labels ((geometry ()
               ...
               (rgb (dx dy)
                 (let* ((d2 (min 1 (+ (* dx dx) (* dy dy))))
                        ...
                        (k2 (* 255 (- 1 d2)))
                        (ir (floor (* colv (+ (* r d2) k2))))
                        (ig (floor (* colv (+ (* g d2) k2))))
                        (ib (floor (* colv (+ (* b d2) k2)))))
                   (list ir ig ib)))
