      ...
      (dotimes (x 120)
        (let ((p (logxor [x] *color*)))
          (cond
            ...
            ((= p +KNIGHT+)
             (incf material +KNIGHT-VALUE+)
             (dolist (d +KNIGHT-DIR+)
               (let ((y (+ x d)))
                 (incf space (space *color* y +OPP+PAWN+)))))
            ((= p #.(+ +COLOR+ +KNIGHT+))
             (decf material +KNIGHT-VALUE+)
             (dolist (d +KNIGHT-DIR+)
               (let ((y (+ x d)))
                 (decf space (space opponent y +MY+PAWN+)))))
            ((= p +BISHOP+)
             (incf material +BISHOP-VALUE+)
             (dolist (d +BISHOP-DIR+)
               (do ((y (+ x d) (+ y d)))
                   ((/= [y] +EMPTY+)
                      (incf space (space *color* y +OPP+PAWN+)))
