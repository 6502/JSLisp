...
(defun input-edit (b hv-node cback)
  (let** ((w (window 0 0 300 230 title: "Input properties"))
          ...
          (#'ok ()
            ...
            (hide-window w)
            (funcall cback)))
    (autofill caption name (lambda (x) (replace x "[-_]" " ")))
    (setf (text caption) (caption b))
    (setf (text name) (or b.data-name ""))
...
(defun edit-window-properties (window-properties)
  (let** ((w (window 0 0 361 232 title: "window properties"))
          (title (add-widget w (input "title" autofocus: true)))
          (left (add-widget w (input "left")))
          (top (add-widget w (input "top")))
          (width (add-widget w (input "width")))
          (height (add-widget w (input "height")))
          (center (add-widget w (checkbox "center")))
          (modal (add-widget w (checkbox "modal")))
          (ok (add-widget w (button "OK" #'ok)))
          (cancel (add-widget w (lbutton "Cancel" (hide-window w))))
          (#'ok ()
            (setf window-properties.title (text title))
            (setf window-properties.left (text left))
            (setf window-properties.top (text top))
            (setf window-properties.width (text width))
            (setf window-properties.height (text height))
            (setf window-properties.center (checked center))
            (setf window-properties.modal (checked modal))
            (hide-window w)))
    (set-layout w (V border: 8 spacing: 8
                     size: 40
                     (dom title)
                     (H spacing: 8
                        (dom left) (dom top) (dom width) (dom height))
                     size: 20
                     (dom center)
                     (dom modal)
                     :filler:
                     size: 30
                     (H spacing: 8 :filler: size: 80 (dom ok) (dom cancel) :filler:)))
    (setf (text title) window-properties.title)
    (setf (text left) window-properties.left)
    (setf (text top) window-properties.top)
    (setf (text width) window-properties.width)
    (setf (text height) window-properties.height)
    (setf (checked center) window-properties.center)
    (setf (checked modal) window-properties.modal)
...
(defun gui-editor (code-cback)
  (let** ((w (window 0 0 0.75 0.75 title: "GUI editor"))
          ...
          (#'node-click (n)
            (cond
              ((= n.text "Window")
               (edit-window-properties window-properties))
              (n.propedit
    ...
    (add-widget-button "Button" (lambda () (button "" (lambda ()))) 80 30 #'button-edit #'button-code)
    (add-widget-button "Input" (lambda () (input "")) 200 40 #'input-edit #'input-code)
    (add-widget-button "Select" (lambda () (select "" (range 10))) 200 40 #'select-edit #'select-code)
    (add-widget-button "Checkbox" (lambda () (checkbox "")) 200 40 #'checkbox-edit #'checkbox-code)
    (add-widget-button "Radio" (lambda () (radio 1 "")) 200 40 #'radio-edit #'radio-code)
    (add-widget-button "Textarea" (lambda () (text-area "")) 200 80 #'textarea-edit #'textarea-code)
    (add-widget-button "Color" (lambda () (css-color-input "")) 200 40 #'color-edit #'color-code)
    (add-widget-button "Date" (lambda () (date-input "")) 120 40 #'date-edit #'date-code)
